...
export default function Notifications() {
    ...
    return (
        <div className="animate-fadeIn pb-12" style={{ paddingRight: isMobile ? 0 : 20 }}>
            ...
            <div style={{ 
                ...
                <div style={{ display: 'none', height: 0, overflow: 'hidden' }}>
                    ...
                    <h1 style={{ fontSize: isMobile ? '1.4rem' : '1.6rem', fontWeight: 950, color: 'var(--navy-900)', margin: 0, letterSpacing: '-0.02em' }}>Notification Center</h1>
                    <p style={{ fontSize: '0.8rem', color: 'var(--slate-500)', fontWeight: 600, marginTop: 2 }}>Unified telemetry & dispatch</p>
                </div>
                <div style={{ display: 'flex', gap: 12 }}>
                    <button className="btn hover-lift" onClick={() => setShowComposer(true)} style={{ 
                        background: 'linear-gradient(135deg, var(--navy-900), var(--navy-700))', 
                        color: 'white', border: 'none', padding: isMobile ? '12px 20px' : '10px 20px', 
                        borderRadius: 12, width: isMobile ? '100%' : 'auto',
            ...
            <div style={{ 
                ...
                {[
                    { label: 'Total Logs', value: statsTotal || log.length, trend: 'Unified', icon: <Send size={18} />, color: 'var(--navy-600)' },
                    { label: 'Network', value: delivered, trend: `${Math.round((delivered / safeTotal) * 100)}% Arrival`, icon: <CheckCircle size={18} />, color: '#10b981' },
                    { label: 'WhatsApp', value: whatsappCount, trend: 'Live', icon: <MessageSquare size={18} />, color: '#25d366' },
                    { label: 'Legacy', value: emailCount + smsCount, trend: 'Mail/SMS', icon: <Smartphone size={18} />, color: 'var(--accent-cyan-dark)' },
                ].map((k, i) => (
                    <div key={k.label} className="ent-card hover-lift" style={{ 
                        padding: isMobile ? '12px' : '10px 18px', position: 'relative', overflow: 'hidden', 
                        display: 'flex', flexDirection: 'column', justifyContent: 'center', height: isMobile ? 80 : 72,
                        minWidth: 0, flex: 1
                    }}>
                        ...
                        <div style={{ fontSize: '1.2rem', fontWeight: 950, color: 'var(--navy-900)', lineHeight: 1 }}>{k.value}</div>
                        <div style={{ fontSize: '0.6rem', fontWeight: 800, color: k.color, marginTop: 4 }}>{k.trend}</div>
                    </div>
                ))}
            </div>
                ...
                <div style={{ 
                    ...
                    <div style={{ position: 'relative', flex: 1 }}>
                        ...
                        <input
                            ...
                            style={{ paddingLeft: 40, height: 42, borderRadius: 12, border: '1px solid var(--border-medium)', fontSize: '0.82rem', fontWeight: 600, width: '100%' }}
                        />
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                        <select className="form-control" style={{ flex: 1, height: 42, borderRadius: 12, fontWeight: 700, background: 'white', fontSize: '0.8rem' }} value={filterChannel} onChange={e => setFilterChannel(e.target.value)}>
                            <option value="all">Channel: All</option>
                            <option value="whatsapp">WhatsApp</option>
                            <option value="email">Mail</option>
                            <option value="sms">SMS</option>
                        </select>
                        <select className="form-control" style={{ flex: 1, height: 42, borderRadius: 12, fontWeight: 700, background: 'white', fontSize: '0.8rem' }} value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                    ...
                    <div style={{ fontSize: '0.62rem', fontWeight: 900, color: 'var(--slate-400)', textTransform: 'uppercase', letterSpacing: '0.04em', textAlign: 'center' }}>
                        {filtered.length} RECORDS ACCESSED
                    </div>
                </div>
            </div>
            ...
            {!isMobile ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {paginated.map(n => {
                        ...
                        const st = STATUS_STYLE[n.status] || STATUS_STYLE.delivered;
                        return (
                            <div key={n.id} className="ent-card" style={{ padding: '16px', border: '1px solid var(--border-light)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                                    <div style={{
                                        ...
                                        {chTheme.icon} {chTheme.label.toUpperCase()}
                                    </div>
                                    <div style={{
                                        display: 'inline-flex', alignItems: 'center', gap: 6,
                                        color: st.color, fontWeight: 900, fontSize: '0.65rem'
                                    }}>
                                        {st.icon} {st.label.toUpperCase()}
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'var(--navy-600)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '0.8rem' }}>
                                        {n.recipient[0]}
                                    </div>
                                    <div>
                                        <div style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--navy-900)' }}>{n.recipient}</div>
                                        <div style={{ fontSize: '0.7rem', color: 'var(--slate-500)' }}>{n.sentAt}</div>
                                    </div>
                                </div>
                                <div style={{ 
                                    background: 'var(--slate-50)', padding: '10px', borderRadius: 10, 
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ fontSize: '0.65rem', color: 'var(--slate-400)', fontWeight: 700 }}>BY: {n.sentBy}</div>
                                    <button 
                                        onClick={() => setShowComposer(true)}
                                        style={{ border: 'none', background: 'none', color: 'var(--navy-600)', fontWeight: 900, fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: 4 }}
                                    >
                                        <Send size={12} /> RESEND
                                    </button>
                                </div>
            ...
            {filtered.length === 0 && (
                <div style={{ padding: '80px 0', textAlign: 'center' }}>
                    ...
                    <h3 style={{ fontSize: '1.2rem', fontWeight: 950, color: 'var(--navy-900)', margin: 0 }}>Log Empty</h3>
                    <p style={{ fontSize: '0.9rem', color: 'var(--slate-400)', marginTop: 8 }}>No matching records.</p>
                </div>
            )}
            ...
            {filtered.length > 0 && (
                <div style={{ 
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--slate-500)', fontWeight: 700, textAlign: 'center' }}>
                        Showing {Math.min(filtered.length, (currentPage - 1) * PAGE_SIZE + 1)} - {Math.min(currentPage * PAGE_SIZE, filtered.length)} of <span style={{ color: 'var(--navy-900)', fontWeight: 900 }}>{filtered.length}</span> Records
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                        <button 
                            ...
                            style={{ 
                                padding: '8px 16px', borderRadius: 12, background: 'white', 
