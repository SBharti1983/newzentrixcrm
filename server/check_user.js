...
async function main() {
    ...
    const userRes = await pool.query(
        "SELECT id, name, email, role, is_active, tenant_id, created_at FROM users WHERE email = 'mmproperties0604@gmail.com'"
    );
    out += '=== USER SEARCH (mmproperties0604@gmail.com) ===\n';
    out += JSON.stringify(userRes.rows, null, 2);

    const allUsersRes = await pool.query(
        "SELECT u.name, u.email, u.role, u.is_active, u.tenant_id, t.name as tenant_name FROM users u JOIN tenants t ON u.tenant_id = t.id ORDER BY t.name, u.name"
    );
    out += '\n\n=== ALL USERS ===\n';
