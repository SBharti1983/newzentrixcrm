...
const recordAudit = async (action, targetId, details = {}) => {
    try {
        await pool.query(
...
router.post('/tenants/:id/nudge', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/utilization-alerts', async (req, res) => {
    try {
        const query = `
...
router.get('/subscriptions/:id/invoice', async (req, res) => {
    try {
        const { id } = req.params;
        const { rows: [sub] } = await pool.query(`
            SELECT s.*, t.name as tenant_name, t.slug as tenant_slug
