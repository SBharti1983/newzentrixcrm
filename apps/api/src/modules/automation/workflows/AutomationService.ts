...
import * as notifier from '../../system/notifier';
import rohanBridgeClient from '../../ai/rohanBridge/RohanBridgeClient';
...
class AutomationService {
    ...
    async handlePostSiteVisitFeedback(lead, io) {
        ...
        try {
            // 1. Generate Personalized AI Message — prefer Rohan's memory-aware followup
            let message = '';
            try {
                const rohanResponse = await rohanBridgeClient.followup({
                    tenant_id: Number(tenant_id),
                    lead_id: String(lead_id),
                    lead_name: name,
                    nurture_reason: `Post site-visit feedback for ${project_name || 'the project'}`,
                    channel: 'whatsapp'
                });
                if (rohanResponse && rohanResponse.message) {
                    message = rohanResponse.message;
                }
            } catch (err) {
                console.warn('[AutomationService] Rohan followup unavailable, falling back to generic AI:', err);
            }

            if (!message) {
                message = await aiService.generateFeedbackRequest(name, project_name || 'the project');
            }
