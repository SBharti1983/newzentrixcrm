...
class ChatbotService {
    ...
    async handleIncomingMessage(tenantId: string | number, fromPhone: string, messageText: string) {
        try {
            ...
            if (bot.ai_enabled) {
                // Try to use Rohan AI Twin first
                try {
                    const rohanResponse = await rohanBridgeClient.chat({
                    ...
                    if (rohanResponse && rohanResponse.message) {
                        responseText = rohanResponse.message;
                    }
                } catch (err) {
                    console.warn('[ChatbotService] AI Rohan twin unavailable, falling back to legacy prompt:', err);
                }

                if (!responseText) {
                    // 3. Fallback: Generate AI Response via Gemini
                    const systemPrompt = `
