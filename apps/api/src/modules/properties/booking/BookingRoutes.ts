...
router.get('/:id', async (req: any, res: Response) => {
    try {
        // 🔥 STORED PROCEDURE: Booking Details + Installments in 1 trip
        const spRes = await pool.query('SELECT get_booking_details($1::uuid, $2::uuid) as data', [req.params.id, req.tenantId]);
...
router.patch('/:id/installments/:iid', async (req: any, res: Response) => {
    try {
