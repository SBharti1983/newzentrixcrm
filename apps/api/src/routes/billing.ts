...
// Plan configuration
const PLANS: Record<string, { priceId: string; amount: number; name: string; limits: { max_users: number; max_leads: number; max_projects: number } }> = {
    starter: {
...
router.post('/subscribe', authenticateToken, async (req: any, res: Response) => {
    ...
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            subscription_data: { metadata: { tenantId, planKey: plan } }
        });
 ...
 * ─── RAZORPAY FLOW ────────────────────────────────────────────────
 */
router.post('/razorpay/order', authenticateToken, async (req: any, res: Response) => {
    const { plan } = req.body;
    const tenantId = req.tenantId;
...
router.post('/razorpay/verify', authenticateToken, async (req: any, res: Response) => {
...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req: any, res: Response) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        try {
            ...
            await client.query(`
                UPDATE tenants 
                SET plan = $1, max_users = $2, max_leads = $3, max_projects = $4, stripe_subscription_id = $5
                WHERE id = $6
...
router.get('/referral', authenticateToken, async (req: any, res: Response) => {
    try {
