...
### 1.1 Extract a shared `BaseCognitiveLoop` (HIGH priority) — ✅ DONE
...
### 1.2 Fix Track B error handling inconsistency (HIGH priority) — ✅ DONE
**Problem**:
...
### 1.3 Add Track B timeout + cancellation (MEDIUM priority) — ✅ DONE
...
### 1.4 Deduplicate the sentence-streaming buffer logic (MEDIUM priority) — ✅ DONE
...
### 1.5 Add per-turn metrics emission (LOW priority) — ✅ DONE
...
### 2.1 Extract `BasePersonaEngine` (HIGH priority) — ✅ DONE
...
### 2.2 Cache invalidation on DB write (MEDIUM priority) — ✅ DONE
...
### 2.3 Multi-tenant cache key bug (MEDIUM priority) — ✅ DONE
...
**Implementation**:
- [`PersonaCacheEntry`](apps/digital-employee/src/employees/BasePersonaEngine.ts:33) now stores `cached_updated_at: Date` (snapshot of the persona row's `updated_at` at cache-fill time).
- Added `CACHE_FRESHNESS_PROBE_MS = 30s`. Within this window after cache-fill, the entry is trusted without a DB probe (the Redis `persona:updated` invalidation from item 2.2 covers it).
- Beyond the freshness window but within the 5-min TTL, [`getPersona`](apps/digital-employee/src/employees/BasePersonaEngine.ts:95) issues a cheap `SELECT updated_at FROM ai_employee_personas WHERE tenant_id=$1 AND role=$2` probe:
  - If `updated_at` matches the cached snapshot → bump `cached_at` (resets the freshness window) and return the cached row (no full-row fetch).
  - If `updated_at` changed → log the mismatch and fall through to a full `SELECT *` re-fetch.
  - If the row vanished → drop the cache and throw `PersonaNotFoundError`.
  - If the probe itself errors (transient DB issue) → serve the stale cached persona rather than failing the turn (safe default; logged at warn level).
- Verified clean: `npx tsc --noEmit` in `apps/digital-employee` → EXIT 0.

### 2.4 Prompt template injection safety (LOW priority) — ✅ DONE
...
### 3.1 Unify streaming across all three adapters (HIGH priority) — ✅ DONE (via 1.4)
...
### 3.2 Filler audio cache is process-local (MEDIUM priority) — ✅ DONE
...
### 3.3 Speculative LLM has no abort on barge-in (MEDIUM priority) — ✅ DONE
...
**Implementation**:
- [`BaseCognitiveLoop.processCycle`](apps/digital-employee/src/agent/BaseCognitiveLoop.ts:335) now accepts `signal?: AbortSignal` and threads it into `generateAIResponseStream` / `generateAIResponse` (Track A).
- [`streamSentences`](apps/digital-employee/src/cognition/SentenceStreamer.ts:80) accepts `signal` in options, breaks the loop on abort, swallows `AbortError`, and skips the final flush on abort.
- [`BaseVoiceAdapter`](apps/digital-employee/src/voice/BaseVoiceAdapter.ts:113) session gained `speculativeAbortController?: AbortController | null`; `startSpeculative` creates one, stores it on the session, and passes `signal` to `runCognitiveLoop`. The `.then`/`.catch` handlers short-circuit on `signal.aborted` and log `Speculative LLM aborted by barge-in`.
- `handleBargeIn` now calls `session.speculativeAbortController.abort()` before nulling state; `resetTurn` clears the controller.
- All three voice adapters (Rohan, Monika, Neha) `runCognitiveLoop` accept and forward `signal`. The non-speculative `handleFinal` path intentionally does **not** pass a signal (turn is finalized, no barge-in expected).
- Verified clean: `npx tsc --noEmit` in `apps/digital-employee` → EXIT 0.

### 3.4 `waitForSpeculative` busy-polls (LOW priority) — ✅ DONE
...
### 3.5 No concurrent-call limit per tenant (LOW priority) — ✅ DONE
...
### 4.1 Reasoning output is unvalidated `as` cast (HIGH priority) — ✅ DONE
...
### 4.2 Reasoning prompt is rebuilt every turn (MEDIUM priority) — ✅ DONE
...
### 4.3 No reasoning retry on schema failure (MEDIUM priority) — ✅ DONE
...
### 4.4 Reasoning doesn't use conversation history embeddings (LOW priority) — ✅ DONE
...
**Implementation**:
- **Rohan** (reference): [`buildKnowledgeBlock`](apps/digital-employee/src/employees/Rohan/Persona.ts:97) already renders `context.semantic_memories` into a `RELEVANT PAST CONTEXT (RAG):` block (lines 163-177).
- **Monika**: Added `buildReasoningContextBlock(context: MonikaContext)` to [`MonikaPersonaEngine`](apps/digital-employee/src/employees/Monika/Persona.ts:158). The reasoning branch of `buildSystemPrompt` now appends this block, rendering `last_reasoning` (intent/emotion/action/next_goal/handoff) and `semantic_memories` (RAG, each wrapped in `userDataBlock('PastTurn', …)` for injection safety per item 2.4).
- **Neha**: Added the same `buildReasoningContextBlock(context: NehaContext)` to [`NehaPersonaEngine`](apps/digital-employee/src/employees/Neha/Persona.ts:90). Renders `AccountantReasoningOutput` fields (intent/emotion/action/query_summary/next_goal/handoff) plus the RAG block. Import updated to include `userDataBlock`.
- All three employees now have RAG parity in the reasoning track. Vector recall is populated upstream by [`retrieveSimilarConversations`](packages/rag/src/conversationMemory.ts:126) into `context.semantic_memories`.
- Verified clean: `npx tsc --noEmit` in `apps/digital-employee` → EXIT 0.

### 4.5 Emotion score is hardcoded 0.5 fallback (LOW priority) — ✅ DONE
...
| Priority | Item | Subsystem | Effort | Status |
|----------|------|----------|--------|--------|
...
All 19 items across all 6 phases are **implemented and verified**:

| Phase | Items | Status |
|-------|-------|--------|
| 1. Foundation refactor | 1.1, 2.1, 1.4 | ✅ `BaseCognitiveLoop.ts`, `BasePersonaEngine.ts`, `SentenceStreamer.ts` exist |
| 2. Correctness | 1.2, 4.1 | ✅ `validateReasoningOutput` / `normalizeReasoning` / `buildFallbackReasoning` in base + all 3 agents |
| 3. Reliability | 1.3, 3.3, 4.3 | ✅ `REASONING_TIMEOUT_MS` race, `speculativeAbortController`, `repairReasoning` |
| 4. Performance | 4.2, 3.2, 2.2 | ✅ `loadPrompt` memoized, Redis L2 filler cache, `persona:updated` Redis pub/sub |
| 5. Observability | 1.5, 4.5 | ✅ `employee:*` events via `EventBroadcaster`, `confidence: null` vs numeric |
| 6. Hardening | 2.4, 3.4, 3.5, 4.4 | ✅ `renderTemplate`/`sanitizeUserField`/`userDataBlock`, event-driven `waitForSpeculative`, `MAX_CONCURRENT_SESSIONS_PER_TENANT`, RAG in all 3 reasoning tracks |

**Verification**: `npx tsc --noEmit` in `apps/digital-employee` → EXIT 0 (clean).

### Suggested next directions (not part of this plan)
- Runtime/integration tests for the new `BaseCognitiveLoop` paths (Track B timeout, repair retry, abort on barge-in).
- Load-test the Redis L2 filler cache hit rate under multi-pod scaling.
- Add a CRM admin UI hook to `NOTIFY`/publish `persona:updated` on persona edits (server-side pub already wired; verify the admin write path fires it).
- Consider extracting a `@zentrix/ai` package from `apps/digital-employee/src/ai` (LLMProvider, ModelRouter, AIService) per the AGENTS.md layered-dependency rules, so the worker can reuse routing without importing the whole app.
